...
class State {
	...
	_won(col, row, piece) {
		...
		if (this.board.map(vert=>vert[row]||'0').join('').includes(win_string)) return true;
		...
		//  o......   c = 0, r = 1 
		//  .......
		if (this.board.map((vert,c)=>vert[c+row-col]||'0').join('').includes(win_string)) return true;
		...
		//  ....x.. < col=4, row=1
		//  .....o.   c = 5, r = 0
		if (this.board.map((vert,c)=>vert[col+row-c]||'0').join('').includes(win_string)) return true;
